...
const drawSelectionPlugin = ViewPlugin.fromClass(class {
  ...
  readPos(): Measure {
    ...
    let rangePieces = state.selection.ranges.map(r => measureRange(this.view, r)).reduce((a, b) => a.concat(b))
    let cursors = []
...
function measureCursor(view: EditorView, cursor: SelectionRange, primary: boolean): Piece | null {
  ...
  if (fatCursor) {
    let nextPos = view.coordsAtPos(head +1, cursor.assoc || 1)
    let w = nextPos ? nextPos.left - pos.left : view.defaultCharacterWidth;
    return new Piece(pos.left - base.left, pos.top - base.top, w, (pos.bottom - pos.top) * hCoeff,
